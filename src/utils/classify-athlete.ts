/**
 * Per-lift athlete classification (browser-compatible).
 *
...
const MALE_THRESHOLDS = {
  back_squat: [1.25, 1.85] as const,
  bench_press: [0.9, 1.35] as const,
  deadlift: [1.4, 2.1] as const,
};
...
function strengthLevel(ratio: number, thresholds: readonly [number, number]): string {
...
export function classifyAthlete(input: ClassifyInput): AthleteLevels {
  ...
  if (bwLbs) {
    if (squat) {
      ...
      squat_level = strengthLevel(squat / bwLbs, t);
    }
    if (bench) {
      const t: [number, number] = [
      ...
      bench_level = strengthLevel(bench / bwLbs, t);
    }
    if (dead) {
      const t: [number, number] = [
      ...
      deadlift_level = strengthLevel(dead / bwLbs, t);
    }
  }
  ...
  if (squat && squat > 0) {
    ...
    const cjLbs = lifts.clean_and_jerk ? toLbs(lifts.clean_and_jerk, units) : null;

    if (snatchLbs) {
      snatch_level = snatchLbs / squat >= OLY_THRESHOLDS.snatch ? 'B' : 'A';
    }
    if (cjLbs) {
      clean_jerk_level = cjLbs / squat >= OLY_THRESHOLDS.clean_and_jerk ? 'B' : 'A';
    }
  }

  return { squat_level, bench_level, deadlift_level, snatch_level, clean_jerk_level };
}
