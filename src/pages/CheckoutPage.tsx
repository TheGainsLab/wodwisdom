  ...
  const selectPlan = async (p: PlanKey) => {
    ...
    if (!hasSubscription) {
      ...
      try {
        ...
        if (data?.url) {
          // Strip ?plan from this history entry BEFORE leaving for Stripe, so the
          // browser BACK button from Stripe returns to a clean /checkout (the
          // plan browser, with an escape) instead of re-firing the auto-redirect
          // and bouncing the user straight back to Stripe (the old trap).
          ...
          <div style={{ maxWidth: 520, margin: '0 auto', padding: '24px 0' }}>
            ...
            {stage === 'loading' && (
              <div style={{ textAlign: 'center', padding: '60px 0' }}>
                ...
                <p style={{ color: 'var(--text-dim)', fontSize: 13, margin: '20px 0 0' }}>Taking you to secure checkout…</p>
                <button
                  className="auth-btn"
                  onClick={() => navigate('/')}
            ...
            {stage === 'browse' && (
              <div className="checkout-plans">
                ...
                {/* Always an explicit way out — a new user who doesn't want to
                    subscribe must never be stranded on the checkout. */}
                {!hasSubscription && (
                  <button
                    type="button"
                    className="auth-btn"
                    onClick={() => navigate('/')}
                    style={{ width: '100%', marginTop: 12, background: 'transparent', color: 'var(--text-dim)', border: '1px solid var(--border)' }}
                  >
                    Maybe later
                  </button>
                )}
