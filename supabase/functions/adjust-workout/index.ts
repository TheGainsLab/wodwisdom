...
- NO positional/core drills in metcons (planks, hollow rocks/holds, dead bugs, wall sits, dead hangs, static holds) — they're accessory/skills tools. Rep-counted core work (T2B, GHD Sit-Ups, Sit-Ups, V-Ups) is fine. Exception: if the athlete explicitly asks for one, honor their request.
- Alternating/unilateral movements: sets × reps must split evenly per side — even reps per set, or odd reps with an even set count; NEVER odd sets × odd reps (5×3 alternating cannot split).
