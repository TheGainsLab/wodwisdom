...
COMPOSE LIKE A CRAFTSMAN
  ...
  - LOADS ARE TYPED, NEVER ADJECTIVES: state every loaded movement's load as load_class + load_band from the shared table below. An adjective in prose without the typed fields is a defect. The band is binding on the fill, which computes the exact weight from THIS athlete's parent-lift 1RM — so "moderate" always means the same fraction of their capacity. The same goes for NUMBERS: block_scheme carries NO per-movement quantities at all — no loads ("@195 lbs"), no percentages, no calories ("15 cal Row"), no reps, no distances. The typed movement fields own every quantity; block_scheme is format + clock + rounds pattern + rest only ("AMRAP 13", "3 rounds: 4:00 on / 1:00 off", "21-15-9 for time" — a rounds pattern must exactly match the movements' schemes). Distances anywhere in prose use ft or m, never yards.
