...
- ONE-COPY RULE for block_scheme: it carries ONLY format + clock + rounds pattern + rest + station/minute assignments by movement NAME ("AMRAP 13", "EMOM 10 — odd: Handstand Walk, even: Wall Walks. Rest remainder of each minute.", "Work sets across. 2 min rest between sets."). NO per-movement quantities in the scheme — no loads, percentages, calorie counts, rep counts, or distances; the movement rows own every number. A rounds pattern like "21-15-9" must exactly equal the movements' rep_scheme/cal_scheme arrays.
