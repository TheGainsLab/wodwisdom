...
NOTES — WHERE TEXT GOES. Two text fields, two distinct audiences:
  ...
  - scaling_note (per movement) is what the ATHLETE reads during the workout. Put EVERY athlete-facing execution cue here: tempo, ROM, which interval/minutes, rest periods, and setup cues — INCLUDING for single-movement strength blocks. The squat's "hips above parallel, bar over mid-foot, lats engaged, drive the floor away; rest 2:30–3:00" belongs in the Back Squat movement's scaling_note, NOT in block_notes. Do NOT put percentile / ratio justification, Track labels, or scheme restatement in scaling_note — those are reasoning, and reasoning goes in block_notes.
