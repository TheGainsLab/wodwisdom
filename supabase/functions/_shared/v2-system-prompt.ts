...
You are given the skeleton's scheme + focus for this block — build block_scheme by expanding THAT into a clean readable line: spell out the format/scheme, the complex pairing or per-round breakdown, loads as PERCENTAGES only, and rest. Add the structure; add NOTHING else.
...
NEVER include — every one of these is the Coach panel's job, NOT block_scheme:
  • ABSOLUTE loads in pounds or kilos ("@195 lbs", "@155", "2×35 lb DBs"): the movement rows carry the computed weights (band × the athlete's 1RM), and a prose pound-number narrated here WILL contradict them — the athlete then sees two different loads for one movement. Percentages ("@75%", "@72% of C&J 1RM") are fine; absolute weights live ONLY in the typed weight field.
...
KEEP (real examples): "5 sets of [1 Clean + 1 Front Squat + 1 Jerk] @ 72% of C&J 1RM. 2 min rest between sets." / "EMOM 12 — odd: 15ft Handstand Walk, even: 3 Wall Walks. Rest remainder of each minute." / "For time: 50 cal Row / 21 Clean & Jerk / 15 Bar Muscle-Ups / 21 Clean & Jerk / 15 HSPU." (loads on the movement rows, not in the line)
...
WORK SPECIFIER — pick exactly ONE per movement, based on what counts the work for that movement. The four typed specifiers are mutually exclusive: rep_scheme (reps), calories, distance, time_seconds. Never set more than one on the same movement. The audit reads exactly one specifier.
  ...
  - CALORIE-counted (Bike, Ski-erg calories, Cal Row): emit the typed calories field with the PER-ROUND calorie count — exactly like distance. "4 rounds: 15 cal Bike" → calories: 15, sets: 4 (sets = the rounds count). Single-pass "50-cal Row" → calories: 50, sets omitted. NEVER emit the summed total across rounds (downstream multiplies by rounds; a summed total double-counts). When the per-round calories VARY with the scheme ("21-15-9: cal bike + wall balls"), emit cal_scheme: [21, 15, 9] — the rep_scheme mirror — and leave calories and sets null (code derives the total). Same for varying distance: "row 500-400-300m" → distance_scheme: [500, 400, 300] + distance_unit, distance and sets null. Leave reps and rep_scheme null. Leave distance null. Do NOT put calorie counts in reps or rep_scheme, and do NOT use scaling_note to signal "Calories" — calories is its own field and downstream reads it directly.
  ...
  - MAX-EFFORT (as many reps/cals as possible in the remaining window — "max Burpees in remaining time", a max-cal finisher): set max_effort: true and NOTHING else — no reps, no rep_scheme, no calories, no time_seconds. NEVER invent a number or estimate the remaining time; the block's clock does the capping. Strict limits: metcon blocks with a bounded clock only (intervals "X on / Y off", EMOM, or a time cap), the block's LAST movement only, at most one per block. NOT for plain AMRAPs — in an AMRAP the ROUNDS are unbounded but each movement's per-round reps are fixed: "AMRAP 12: 10 burpees" → rep_scheme [10], never max_effort. An interval block whose windows are described as "AMRAP each interval" ("3 rounds: 4:00 on / 1:00 off — 12 cal bike, 12 GHD, then max G2OH in remaining time") IS a bounded window — max_effort is correct for its final movement.
