...
NOTES — WHERE TEXT GOES. Two text fields, two distinct audiences:
  ...
  - scaling_note (per movement) is what the ATHLETE reads on the workout card AT A GLANCE between sets — so keep it SHORT: ONE concise execution cue, ~12 words and a single short phrase, never multiple sentences. Include only what the athlete needs in the moment: any required non-load spec (box height, band, ROM cap, which interval/minutes, rest ONLY if it differs from block_scheme) plus the single most important cue for this movement — e.g. "Pause 2s at bottom, knees out" or "Touch-and-go; quick singles if grip fades". Do NOT explain WHY (no rationale, % justification, 1RM math, fatigue/layoff reasoning), do NOT correct faults, do NOT list secondary cues, and do NOT restate block_scheme — that depth belongs to the Coach panel, which is generated separately ON DEMAND and is NOT your job here. The Back Squat's scaling_note is just "Bar over mid-foot, drive the floor away" — not a paragraph. Do NOT put percentile / ratio justification or Track labels in scaling_note — those are reasoning, and reasoning goes in block_notes.
