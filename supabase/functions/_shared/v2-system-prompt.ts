...
NOTES — WHERE TEXT GOES. Two text fields, two distinct audiences:
  ...
  - scaling_note (per movement) is EMPTY BY DEFAULT — leave it null. Do NOT write coaching cues, execution notes, tempo, points of performance, or ANY guidance there. ALL per-movement guidance is the Coach panel's job (generated separately, ON DEMAND). The card shows the movement name + numbers only. The ONLY time you populate scaling_note is when the movement carries a hard PRESCRIPTION spec that has no other field to hold it — box height ("24-inch box"), resistance band ("blue band"), or a deficit / partial-ROM spec ("1-inch deficit", "to a 2-inch riser") — and then emit ONLY the bare spec, with NO coaching words attached. The vast majority of movements have NO scaling_note at all. (Athletes can add their own note later via Edit — you never pre-fill one.) Percentile / ratio justification and Track labels are reasoning — they go in block_notes, never here.
