-- Athlete block notes reach next-month generation; block RPE replaces the
-- dead per-set RPE in the previous-cycle summary.
--
-- 1. block_notes: the athlete's own words about how blocks went, typed into
--    the logging Notes field. Newest 30 non-empty notes from the prior
--    program's completed logs, each capped at 500 chars, as
--    [{date, block_type, note}]. CoachState weighs them for month_in_review
--    and load decisions; absence stays NEUTRAL per this function's principle.
--
-- 2. logged_avg_rpe healed: the logging-fidelity redesign (20260808) moved
--    effort to workout_log_blocks.rpe and left workout_log_entries.rpe null
--    on every new log, so AVG(wle.rpe) has been silently null since. Average
--    COALESCE(per-set, block) RPE so old and new logs both count.
--
-- Same single-arg signature as 20260602100000 — CREATE OR REPLACE is safe.
...
BEGIN
  ...
  actuals AS (
    SELECT
      ...
      -- Effort lives on the block since the logging-fidelity redesign; per-set
      -- rpe still counts for pre-redesign logs.
      ROUND(AVG(COALESCE(wle.rpe, wlb.rpe))::numeric, 1) AS logged_avg_rpe,
      ROUND(
        100.0 * COUNT(*) FILTER (
    ...
    JOIN workout_logs wl ON wl.id = wle.log_id
    JOIN program_workouts pw ON pw.id = wl.source_id
    LEFT JOIN workout_log_blocks wlb ON wlb.id = wle.block_id
    WHERE pw.program_id = v_program_id AND wl.status = 'completed'
    GROUP BY 1
  )
  SELECT json_agg(
  ...
  -- BLOCK NOTES: the athlete's own words, newest first, bounded.
  WITH noted AS (
    SELECT
      COALESCE(b.performed_date, wl.workout_date) AS day,
      b.block_type,
      LEFT(TRIM(b.notes), 500) AS note
    FROM workout_log_blocks b
    JOIN workout_logs wl ON wl.id = b.log_id
    JOIN program_workouts pw ON pw.id = wl.source_id
    WHERE pw.program_id = v_program_id
      AND wl.status = 'completed'
      AND b.notes IS NOT NULL AND TRIM(b.notes) <> ''
    ORDER BY COALESCE(b.performed_date, wl.workout_date) DESC, b.sort_order
    LIMIT 30
  )
  SELECT json_agg(
    json_build_object('date', day, 'block_type', block_type, 'note', note)
    ORDER BY day DESC
  )
  INTO v_block_notes
  FROM noted;
  ...
  SELECT json_build_object(
    ...
    'block_notes', COALESCE(v_block_notes, '[]'::json)
  )
  INTO result;
