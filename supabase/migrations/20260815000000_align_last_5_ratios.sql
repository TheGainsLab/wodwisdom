-- Align the repo with production: engine_user_performance_metrics keeps a
-- FIVE-ratio rolling window (last_5_ratios), and the RPC maintains it.
--
-- History: production was widened from a 4- to a 5-ratio window live (column
-- renamed, RPC replaced) and never committed back. The checked-in schema kept
-- saying last_4_ratios, so every READ in the codebase 400'd against the real
-- column — silently: the day page's adaptive pace multiplier fell back to 1.0
-- for all athletes, and the sequencer's conditioning diagnosis lost its
-- competencies section entirely. Writes (this RPC, matched to production)
-- kept working the whole time, so the data is intact.
--
-- On production both statements are no-ops-in-effect: the column is already
-- named last_5_ratios (the DO block skips), and the function body below is
-- byte-equivalent to what is live. On a fresh environment they upgrade the
-- original 20260238/20260241 objects to match production.

DO $$ BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'engine_user_performance_metrics'
      AND column_name = 'last_4_ratios'
  ) THEN
    ALTER TABLE engine_user_performance_metrics
      RENAME COLUMN last_4_ratios TO last_5_ratios;
  END IF;
END $$;
