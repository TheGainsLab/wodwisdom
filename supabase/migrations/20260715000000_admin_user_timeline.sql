-- Admin drill-down: unified per-user activity timeline.
--
-- RPC for /admin/users/:id/timeline (and the Recent Activity preview on the
-- user detail page). One chronological feed built as a UNION ALL over every
-- event-producing table the panel already surfaces separately: account
-- lifecycle, profile edits, evaluations, chat, engine, workouts, nutrition,
-- programs, entitlements, and outbound email. Each event carries a small
-- typed jsonb detail payload (with row ids where a drill-down page exists)
-- so the UI can render and deep-link without extra queries.
--
-- KNOWN LIMIT (by design, documented for readers): this shows what the user
-- DID (content-producing writes), not logins or browsing — there is no app
-- event instrumentation yet. Quiet stretches mean "didn't write anything",
-- not necessarily "wasn't here".
--
-- Pagination: keyset on event_at (p_before). Nutrition is grouped per day
-- (one event per logged day, stamped with that day's last entry) so heavy
-- loggers don't drown the feed. p_types filters to a subset of event types.
-- Fetches p_limit+1 rows to compute has_more without a second scan.
--
-- Authorization: gated on is_current_user_admin(); log_admin_access once per
-- fetch (mirrors the other Gen-2 drill-down RPCs).
  ...
  WITH ev AS (
    ...
    SELECT w.created_at, 'workout_log',
      jsonb_build_object(
        ...
        'score', w.score, 'workout_date', w.workout_date
      )
