-- Fix /admin/ratings: admin_list_rated_messages called row_to_jsonb(x), which
-- does not exist (the Postgres function is to_jsonb). PL/pgSQL resolves the
-- name at first EXECUTION, not at CREATE, so the original migration installed
-- cleanly and the page errored at query time ("function row_to_jsonb(record)
-- does not exist"). One-word fix; everything else is unchanged from
-- 20260430000001_admin_list_rated_messages.sql.
